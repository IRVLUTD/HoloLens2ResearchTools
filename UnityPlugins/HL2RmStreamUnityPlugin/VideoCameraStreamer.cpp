...
#define DBG_ENABLE_ERROR_LOGGING 0
    ...
    long long pTimestamp)
{
    ...
    float ppx = pFrame.VideoMediaFrame().CameraIntrinsics().PrincipalPoint().x;
    float ppy = pFrame.VideoMediaFrame().CameraIntrinsics().PrincipalPoint().y;
    ...
    {
        for (int col = 0; col < rowStride; col += pixelStride)
        {
    ...
    try
    {
        ...
        m_writer.WriteSingle(ppx);
        m_writer.WriteSingle(ppy);
