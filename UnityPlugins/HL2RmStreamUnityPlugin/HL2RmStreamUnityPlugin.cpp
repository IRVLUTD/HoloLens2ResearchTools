...
#define DBG_ENABLE_INFO_LOGGING 0
...
unsigned long long RM_MinDelta = 300000;	// 33 ms ==> 30FPS
std::wstring PVPort = L"10090";
std::wstring AHATPort = L"10091";
...
winrt::Windows::Foundation::IAsyncAction HL2Stream::InitializeVideoFrameProcessorAsync()
{
	...
	m_pVideoFrameStreamer = std::make_shared<VideoCameraStreamer>(m_worldOrigin, PVPort);
...
void HL2Stream::InitializeResearchModeProcessing()
{
	...
	auto ahatStreamer = std::make_shared<ResearchModeFrameStreamer>(AHATPort, guid, m_worldOrigin);
	m_pAHATStreamer = ahatStreamer;

	if (m_pAHATSensor)
	{
		auto processor = std::make_shared<ResearchModeFrameProcessor>(
			m_pAHATSensor, camConsentGiven, &camAccessCheck, RM_MinDelta, m_pAHATStreamer);
